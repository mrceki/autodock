...
from autodock_core_dolly.autodock_utils import DockState
import autodock_core_dolly.autodock_utils as utils
from autodock_core_dolly.autodock_server import AutoDockServer, AutoDockConfig
...
class DefaultAutoDockConfig(AutoDockConfig):
    ...
    """
    # General Configs
    cam_offset = 0        # camera to base link, for edge2edge distance
    front_dock = True
    tf_expiry: float = 3.0      # sec
    ...
    # Fix variables
    left_marker = "dolly_0"
    right_marker = "dolly_0"
    centre_marker = "dolly_0"
    dock = "dolly_0"
    # velocity profiles
    linear_vel_range = (-0.6, 0.6)
    angular_vel_range = (-0.4, 0.4)

    max_linear_vel = 0.3         # m/s, for parallel.c and steer
    min_linear_vel = 0.1        # m/s, for lastmile
    max_angular_vel = 0.30       # rad/s
    min_angular_vel = 0.10       # rad/s

    # predock state
    max_parallel_offset = 0.06    # m, will move to parallel.c if exceeded
...
class AutoDockStateMachine(AutoDockServer):
    ...
    def do_retry(self) -> bool:
        ...
        Attempt to retry the docking again, only retry by retreating if it
            in activate charger, last mile. if Failed with steerDock,
    ...
    def do_predock(self) -> bool:
        ...
        # markers exist again.
        if self.get_dolly_frame() is None:
            return False
        ...
        while not rospy.is_shutdown():
            ...
            centre_tf = self.get_dolly_frame()
            ...
            # publish rotate
            ang_vel = utils.bin_filter(yaw, self.cfg.min_angular_vel) * -1 # Check Me 
    ...
    def do_steer_dock(self) -> bool:
        ...
        while not rospy.is_shutdown():
            ...
            centre_tf = self.get_dolly_frame(offset_from_charger)
